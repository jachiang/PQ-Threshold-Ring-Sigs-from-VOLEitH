...
TEST_CASE( "commit/open/verify", "[vector com]" ) {
    ...
    std::vector<block_secpar> forest(PPRF_VECTOR_COM_NODES);
    ...
    std::vector<unsigned char> opening(PPRF_VECTOR_COM_OPEN_SIZE);
...
#define BATCH_VEC_COM_TEST_MAX_TRIES 1000000000

TEST_CASE( "batch commit/open/verify", "[batch vector com]" ) {

    block_secpar seed = rand<block_secpar>();
    std::vector<block_secpar> forest(BATCH_VECTOR_COMMIT_NODES);
    std::vector<block_secpar> leaves_sender(BATCH_VECTOR_COMMIT_LEAVES);
    std::vector<block_secpar> leaves_receiver(BATCH_VECTOR_COMMIT_LEAVES);
    std::vector<block_2secpar> hashed_leaves_sender(BATCH_VECTOR_COMMIT_LEAVES);
    std::vector<block_2secpar> hashed_leaves_receiver(BATCH_VECTOR_COMMIT_LEAVES);
    std::vector<uint8_t> delta_bytes(SECURITY_PARAM, 0);
    size_t delta[BITS_PER_WITNESS];

	block128 iv = rand<block128>();

    std::vector<unsigned char> opening(BATCH_VECTOR_OPENING_SIZE);

    batch_vector_commit(seed, iv, forest.data(), leaves_sender.data(), hashed_leaves_sender.data());

    #define TEST_REPEATS 100

    bool opening_success = 0;
    size_t tries = 0;

    for (size_t P = 0; P < TEST_REPEATS; P++)
    {
        opening_success = 0;
        while ( opening_success == 0 && tries < BATCH_VEC_COM_TEST_MAX_TRIES)
        {
            tries++;
            // sample new delta
            for (size_t i = 0, dst = 0; i < BITS_PER_WITNESS; ++i)
            {
                size_t k = i < VOLES_MAX_K ? VOLE_MAX_K : VOLE_MIN_K;
                delta[i] = rand<size_t>() % (1 << k);
                expand_bits_to_bytes(&delta_bytes[dst], k, delta[i]);
                dst += k;
            }

            opening_success = batch_vector_open(forest.data(), hashed_leaves_sender.data(), delta_bytes.data(), opening.data());
        }    
    }

    REQUIRE(opening_success == 1);

    bool verify_success = batch_vector_verify(iv, opening.data(), delta_bytes.data(), leaves_receiver.data(), hashed_leaves_receiver.data());

    REQUIRE(verify_success == 1);
    ...
    for (size_t i = 0, src = 0; i < BITS_PER_WITNESS; ++i)
    {
        ...
        for (size_t j = 0; j < (size_t) 1 << k; ++j)
        {
            ...
            arr_secpar sender_leaf, receiver_leaf;
            memcpy(&sender_leaf, &leaves_sender[src + vole_permute_key_index_inv(j)], sender_leaf.size());
            memcpy(&receiver_leaf, &leaves_receiver[src + vole_permute_key_index_inv(j ^ delta[i])], receiver_leaf.size());
            INFO(i);
            INFO(j);
            REQUIRE(sender_leaf == receiver_leaf);
        }
...
TEST_CASE( "compare against tv", "[vector com]" ) {
    ...
    std::vector<block_2secpar> hashed_leaves_receiver(VECTOR_COMMIT_LEAVES);
    std::vector<uint8_t> delta_bytes(SECURITY_PARAM, 0);
    std::vector<uint8_t> opening(PPRF_VECTOR_COM_OPEN_SIZE);
