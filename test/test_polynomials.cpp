...
TEST_CASE( "poly128 from poly64", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly64_vec_xs.size(); offset += 8 * POLY_VEC_LEN) {
        poly64_vec a = poly64_load(enc_poly64_vec_xs.data() + offset);
        poly128_vec b = poly128_load(enc_poly64_vec_as_poly128_xs.data() + 2 * offset);
        poly128_vec a_128 = poly128_from_64(a);
        REQUIRE_POLY128VEC_EQ( a_128, b );
    }
}
...
TEST_CASE( "poly192 from poly128", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly128_vec_xs.size(); offset += 16 * POLY_VEC_LEN) {
        poly128_vec a = poly128_load(enc_poly128_vec_xs.data() + offset);
        poly192_vec b = poly192_load(enc_poly128_vec_as_poly192_xs.data() + (offset / 16) * 24);
        poly192_vec a_192 = poly192_from_128(a);
        REQUIRE_POLY192VEC_EQ( a_192, b );
    }
}
...
TEST_CASE( "poly256 from poly128", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly128_vec_xs.size(); offset += 16 * POLY_VEC_LEN) {
        poly128_vec a = poly128_load(enc_poly128_vec_xs.data() + offset);
        poly256_vec b = poly256_load(enc_poly128_vec_as_poly256_xs.data() + 2 * offset);
        poly256_vec a_256 = poly256_from_128(a);
        REQUIRE_POLY256VEC_EQ( a_256, b );
    }
}
...
TEST_CASE( "poly256 from poly192", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly192_vec_xs.size(); offset += 24 * POLY_VEC_LEN) {
        poly192_vec a = poly192_load(enc_poly192_vec_xs.data() + offset);
        poly256_vec b = poly256_load(enc_poly192_vec_as_poly256_xs.data() + (offset / 24) * 32);
        poly256_vec a_256 = poly256_from_192(a);
        REQUIRE_POLY256VEC_EQ( a_256, b );
    }
}
...
TEST_CASE( "poly384 from poly192", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly192_vec_xs.size(); offset += 24 * POLY_VEC_LEN) {
        poly192_vec a = poly192_load(enc_poly192_vec_xs.data() + offset);
        poly384_vec b = poly384_load(enc_poly192_vec_as_poly384_xs.data() + 2 * offset);
        poly384_vec a_384 = poly384_from_192(a);
        REQUIRE_POLY384VEC_EQ( a_384, b );
    }
}
...
TEST_CASE( "poly320 from poly256", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly256_vec_xs.size(); offset += 32 * POLY_VEC_LEN) {
        poly256_vec a = poly256_load(enc_poly256_vec_xs.data() + offset);
        poly320_vec b = poly320_load(enc_poly256_vec_as_poly320_xs.data() + (offset / 32) * 40);
        poly320_vec a_320 = poly320_from_256(a);
        REQUIRE_POLY320VEC_EQ( a_320, b );
    }
}
...
TEST_CASE( "poly512 from poly256", "[polynomial]" ) {
    for (size_t offset = 0; offset < enc_poly256_vec_xs.size(); offset += 32 * POLY_VEC_LEN) {
        poly256_vec a = poly256_load(enc_poly256_vec_xs.data() + offset);
        poly512_vec b = poly512_load(enc_poly256_vec_as_poly512_xs.data() + 2 * offset);
        poly512_vec a_512 = poly512_from_256(a);
        REQUIRE_POLY512VEC_EQ( a_512, b );
    }
}
...
TEST_CASE("poly128_from_8_poly1", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN)
    {
        ...
        for (size_t bit = 0; bit < 8; ++bit)
            bits[bit] = poly1_load_offset8(&inputs[index], bit);

        poly128_vec combined = poly128_from_8_poly1(bits);
        std::array<std::array<uint8_t, 16>, POLY_VEC_LEN> combinedPacked;
        poly128_store(combinedPacked[0].data(), combined);
...
TEST_CASE("poly192_from_8_poly1", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN)
    {
        ...
        poly192_vec combined = poly192_from_8_poly1(bits);
        std::array<std::array<uint8_t, 24>, POLY_VEC_LEN> combinedPacked;
        poly192_store(combinedPacked[0].data(), combined);
...
TEST_CASE("poly256_from_8_poly1", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN)
    {
        ...
        poly256_vec combined = poly256_from_8_poly1(bits);
        std::array<std::array<uint8_t, 32>, POLY_VEC_LEN> combinedPacked;
        poly256_store(combinedPacked[0].data(), combined);
...
TEST_CASE("poly128_from_8_poly128", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN) {
        poly128_vec polys[8];
        for (size_t bit = 0; bit < 8; ++bit)
            polys[bit] = poly128_load(inputs.data() + (index * 8 + bit) * 16);

        poly128_vec combined = poly128_from_8_poly128(polys);
        std::array<std::array<uint8_t, 16>, POLY_VEC_LEN> combinedPacked;
        poly128_store(combinedPacked[0].data(), combined);
...
TEST_CASE("poly192_from_8_poly192", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN) {
        poly192_vec polys[8];
        for (size_t bit = 0; bit < 8; ++bit)
            polys[bit] = poly192_load(inputs.data() + (index * 8 + bit) * 24);

        poly192_vec combined = poly192_from_8_poly192(polys);
        std::array<std::array<uint8_t, 24>, POLY_VEC_LEN> combinedPacked;
        poly192_store(combinedPacked[0].data(), combined);
...
TEST_CASE("poly256_from_8_poly256", "[polynomial]") {
    ...
    for (size_t index = 0; index < TEST_VEC_LEN; index += POLY_VEC_LEN) {
        poly256_vec polys[8];
        for (size_t bit = 0; bit < 8; ++bit)
            polys[bit] = poly256_load(inputs.data() + (index * 8 + bit) * 32);

        poly256_vec combined = poly256_from_8_poly256(polys);
        std::array<std::array<uint8_t, 32>, POLY_VEC_LEN> combinedPacked;
        poly256_store(combinedPacked[0].data(), combined);
