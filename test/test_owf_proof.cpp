...
TEST_CASE( "ring owf proof", "[ring owf proof]" ) {
    ...
    quicksilver_test_or_state qs_test(OWF_NUM_CONSTRAINTS, reinterpret_cast<uint8_t*>(sk.witness), WITNESS_BITS, delta);
    auto& qs_state_prover = qs_test.prover_state;
    auto& qs_state_verifier = qs_test.verifier_state;
	...
	auto [check_prover, check_verifier] = qs_test.compute_check(active_branch);
    REQUIRE(check_prover == check_verifier);
}