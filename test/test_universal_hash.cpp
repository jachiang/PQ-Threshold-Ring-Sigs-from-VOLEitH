...
TEST_CASE( "hasher gf64", "[universal hash]" ) {
    ...
    for (size_t i = 0; i < NUM_HASHERS; ++i) {
        hasher_gf64_init_key(&hash_key, poly64_load(&key_poly_p[i]));
        hasher_gf64_init_state(&hash_state, NUM_INPUTS);
        for (size_t j = 0; j < NUM_INPUTS; ++j) {
            hasher_gf64_update(&hash_key, &hash_state, poly64_load(&inputs_poly_p[i][j * OFFSET]));
        }
        poly64_vec hash = hasher_gf64_final(&hash_state);
        poly64_vec expected_hash = poly64_load(&expected_outputs_poly_p[i]);
        REQUIRE_POLY64VEC_EQ(hash, expected_hash);
        break;
    }
#else
...
TEST_CASE( "hasher gfsecpar", "[universal hash]" ) {
    ...
    const auto* key_poly_p = enc_keys_256.data();
    const auto* inputs_poly_p = enc_inputs_256.data();
    const auto* expected_outputs_poly_p = expected_enc_outputs_256.data();
#endif
...
TEST_CASE( "hasher gfsecpar_64", "[universal hash]" ) {
    hasher_gfsecpar_64_key hash_key;
    hasher_gfsecpar_64_state hash_state;

    const auto* key_poly_p = enc_keys_64.data();
#if SECURITY_PARAM == 128
    const auto* inputs_poly_p = enc_inputs_128.data();
    const auto* expected_outputs_poly_p = expected_enc_outputs_64x128.data();
#elif SECURITY_PARAM == 192
    const auto* inputs_poly_p = enc_inputs_192.data();
    const auto* expected_outputs_poly_p = expected_enc_outputs_64x192.data();
#elif SECURITY_PARAM == 256
    const auto* inputs_poly_p = enc_inputs_256.data();
    const auto* expected_outputs_poly_p = expected_enc_outputs_64x256.data();
#endif
...
#if POLY_VEC_LEN == 1
    for (size_t i = 0; i < NUM_HASHERS; ++i) {
        hasher_gfsecpar_64_init_key(&hash_key, poly64_load(&key_poly_p[i]));
        hasher_gfsecpar_64_init_state(&hash_state, NUM_INPUTS);
        for (size_t j = 0; j < NUM_INPUTS; ++j) {
            hasher_gfsecpar_64_update(&hash_key, &hash_state, poly_secpar_load(&inputs_poly_p[i][j * OFFSET]));
        }
        poly_secpar_vec hash = hasher_gfsecpar_64_final(&hash_state);
        poly_secpar_vec expected_hash = poly_secpar_load(&expected_outputs_poly_p[i]);
#if SECURITY_PARAM == 128
        REQUIRE_POLY128VEC_EQ(hash, expected_hash);
#elif SECURITY_PARAM == 192
