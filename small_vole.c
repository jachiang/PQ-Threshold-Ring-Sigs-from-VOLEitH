...
static ALWAYS_INLINE void process_prg_output(
	...
	const prg_vole_block* raw_prg_output, size_t col_len)
{
	...
	const uint8_t* restrict delta, size_t col_len)
{
	...
	if (receiver)
	{
		...
		process_prg_output(true, 0, 0, accum, vq, raw_prg_output, col_len);

		// for (size_t j = 1; j < COL_LEN; ++j)
		for (size_t j = 1; j < col_len; ++j)
		{
			prg_vole_gen(prgs, fixed_key, VOLE_WIDTH - 1, PRG_VOLE_BLOCKS, j * PRG_VOLE_BLOCKS, raw_prg_output);
			process_prg_output(true, j, 0, accum, vq, raw_prg_output, col_len);
		}
	...
	for (; i < (size_t) 1 << k; i += VOLE_WIDTH)
	{
		...
		process_prg_output(false, 0, output_col, accum, vq, raw_prg_output, col_len);

		// for (size_t j = 1; j < COL_LEN; ++j)
		for (size_t j = 1; j < col_len; ++j)
		{
			prg_vole_gen(prgs, fixed_key, VOLE_WIDTH, PRG_VOLE_BLOCKS, j * PRG_VOLE_BLOCKS, raw_prg_output);
			process_prg_output(false, j, output_col, accum, vq, raw_prg_output, col_len);
		}
	}
...
{
	vole(false, k, keys, iv, fixed_key, u, v, c, NULL, col_len);
}
...
void vole_receiver(
	...
	const uint8_t* restrict delta, size_t col_len)
{
	vole(true, k, keys, iv, fixed_key, c, q, NULL, delta, col_len);
}
